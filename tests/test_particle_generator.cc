...
TEST_CASE("SetBox", "[SetBox]") {
    ParticleGenerator pg;
    Box box(vec2(0,0), 1, 1);
    pg.SetBox(box);
    REQUIRE(box.GetBoxSize() == 1);
    vec2 check(0,0);
    REQUIRE(box.GetTopLeftCorner() == check);
    REQUIRE(box.GetParticleRadius() == 1);
}

TEST_CASE("Set and Get Particle Radius", "[SetParticleRadius][GetParticleRadius]") {
    ParticleGenerator pg;
    pg.SetParticleRadius(15);
    REQUIRE(pg.GetParticleRadius() == 15);
}

TEST_CASE("UpdateParticles", "[UpdateParticles][SetBox][GenerateParticle][GetParticleList]") {
    ParticleGenerator pg;
    Box box(vec2(0,0), 50, 1);
    pg.SetBox(box);
